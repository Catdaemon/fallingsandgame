    ...
    public class MgFrameRate
    {
        ...
        )
        {
            ...
            if (fixedon)
            {
                ...
                pass_in_this.TargetElapsedTime = TimeSpan.FromTicks(
                    (long)(TimeSpan.TicksPerSecond / desiredFramesPerSecond)
                );
            }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (elapsed >= DisplayedMessageFrequency) // || (gcDiff != 0)
            {
                ...
                if (DisplayFrameRate)
                {
                    ...
                    fpsmsg
                        .Append(" Interval error: ")
                        .Append(elapsed - DisplayedMessageFrequency)
                        .AppendLine();
                ...
                if (DisplayGarbageCollectionRate)
                {
                    ...
                    gcmsg.Clear();
                    gcmsg
                        .Append(" GC Memory(Mb) Now: ")
                        .AppendTrim((double)gcNow / MEGABYTE)
                        .AppendLine();
                    ...
                    {
                        gcmsg
                            .AppendLine(" !!! COLLECTION OCCURED !!! ")
                            .Append(" GC Memory(Mb) Lost: ")
                            .AppendTrim((double)(gcDiff) / MEGABYTE)
                            .AppendLine();
                        var tmp = -gcDiff;
                    ...
                    gcmsg
                        .Append(" GC Memory(Mb) AccumulatedInc: ")
                        .AppendTrim(gcAccumulatedInc / MEGABYTE, 6)
                        .AppendLine();
                    if (gcRecordedCollects > 0)
                    {
                        gcmsg
                            .Append(" GC Memory(Mb) Accumulated Since Collect: ")
                            .AppendTrim(gcAccumulatedSinceLastCollect / MEGABYTE)
                            .AppendLine();
                        gcmsg
                            .Append(" GC Memory(Mb) Last Lost Collection: ")
                            .AppendTrim(gcSizeOfLastCollect / MEGABYTE)
                            .AppendLine();
                        gcmsg
                            .Append(" GC Memory(Mb) Total Lost to Collections: ")
                            .AppendTrim(gcTotalLost / MEGABYTE)
                            .AppendLine();
                    }
                    gcmsg
                        .Append(" GC Number Of Memory Collections: ")
                        .AppendTrim(gcRecordedCollects)
                        .AppendLine();
                }
                msg.Clear();
        ...
        )
        {
            ...
            if (DisplayCollectionAlert && gcRecordedCollects > 0)
                spriteBatch.DrawString(
                    ...
                    msg,
                    fpsDisplayPosition,
                    new Color(1f, fpsTextColor.G, fpsTextColor.B, fpsTextColor.A)
                );
            else
        ...
        private void DrawVisualizations(Color fpsTextColor)
        {
            ...
            var vmsrheight = 4;
            var vmsrWidth = 30;
            Rectangle visualMotionStutterRect = new Rectangle(
                (int)(dist * elapsed) + 5,
                2,
                vmsrWidth,
                vmsrheight
            );
            Rectangle visualMotionStutterRect2 = new Rectangle(
                (int)(dist - (dist * elapsed)) + 5,
                2,
                vmsrWidth,
                vmsrheight
            );
            ...
            if (visualFrame > 120)
            {
                visualFrame = 120d + (visualFrame / 20d);
            }
            ...
            DrawRaySegment(
                ...
                15,
                2,
                (float)((elapsed / DisplayedMessageFrequency) * Math.PI * 2d),
                Color.Gray
            );
        ...
        public static Texture2D TextureFromColorArray(
        ...
        public void DrawSquare(Rectangle r, int lineThickness, Color c)
        {
            ...
            Rectangle BLtoR = new Rectangle(
                r.Left,
                r.Bottom - lineThickness,
                r.Width,
                lineThickness
            );
            Rectangle LTtoB = new Rectangle(r.Left, r.Top, lineThickness, r.Height);
            Rectangle RTtoB = new Rectangle(
                r.Right - lineThickness,
                r.Top,
                lineThickness,
                r.Height
            );
        ...
        {
            rot += 3.141592f;
            Rectangle screendrawrect = new Rectangle(
                (int)postion.X,
                (int)postion.Y,
                linethickness,
                length
